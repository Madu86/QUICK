...
__device__ __forceinline__ int lefthrr_spd(const QUICKDouble RAx, const QUICKDouble RAy, const QUICKDouble RAz,
                                       const QUICKDouble RBx, const QUICKDouble RBy, const QUICKDouble RBz,
                                       const int KLMNAx, const int KLMNAy, const int KLMNAz,
                                       ...
                                       int IJTYPE,QUICKDouble* coefAngularL, unsigned char* angularL)
{
    ...
    if (KLMNBx >= 1) {
        angularL[numAngularL] = LOC3(devTrans, KLMNAx+1, KLMNAy, KLMNAz, TRANSDIM, TRANSDIM, TRANSDIM);
        coefAngularL[numAngularL] = KLMNBx * quick_pow(tmpx, KLMNBx-1) * tmpy2 * tmpz2;
        numAngularL++;
    }
    
    if (KLMNBy >= 1) {
        angularL[numAngularL] = LOC3(devTrans, KLMNAx, KLMNAy+1, KLMNAz, TRANSDIM, TRANSDIM, TRANSDIM);
        coefAngularL[numAngularL] = KLMNBy * quick_pow(tmpy, KLMNBy-1) * tmpx2 * tmpz2;
        numAngularL++;
    }
    if (KLMNBz >= 1) {
        angularL[numAngularL] = LOC3(devTrans, KLMNAx, KLMNAy, KLMNAz+1, TRANSDIM, TRANSDIM, TRANSDIM);
